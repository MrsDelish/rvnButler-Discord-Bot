...
exports.server = {
  ...
  process: function(bot,msg,suffix){
    ...
    var Members = data.members.size;
    var Online = data.members.filter(m => m.presence.status === 'online').size;
    ...
    for (var i = 1; i <= Number(Roles.size)-1; ++i) {
      RolesArray.push(RolesList[i] + ": " + Roles.find('name', RolesList[i]).members.size)
    }
    ...
    msg.channel.send("cannot use ths commad here").then(message => message.delete(10000));
  }
  }
}
