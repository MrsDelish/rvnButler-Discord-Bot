...
let TimedStatsChannel = config.get("Channels").timedstats;
let statsurl = "https://coinmarketcap.com/currencies/ravencoin/";
...
exports.TimedStats = function(bot) {
  ...
  function sendInfo(bot) {
    ...
    needle.get(
      "https://api.coinmarketcap.com/v1/ticker/ravencoin/",
      function(error, response) {
        if (error || response.statusCode !== 200) {
          bot.channels
            .get(TimedStatsChannel)
            .send("coinmarketcap API is not available");
        } else {
          ...
          needle.get(
            "https://api.coinmarketcap.com/v1/ticker/ravencoin/?convert=GBP",
            function(error, response) {
              if (error || response.statusCode !== 200) {
                bot.channels
                  .get(TimedStatsChannel)
                ...
                needle.get(
                  "https://api.coinmarketcap.com/v1/ticker/ravencoin/?convert=EUR",
                  function(error, response) {
                    if (error || response.statusCode !== 200) {
                      bot.channels
                        .get(TimedStatsChannel)
                      ...
                      const embed = {
                        ...
                        author: {
                          name: "Coin Market Cap Stats (RVN)",
                          url: statsurl,
